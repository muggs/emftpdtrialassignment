...
class FakeFTPDriver
  ...
  $bkt = 'em-ftpd-trial/mugdha'                          #This is bucket name
 ...
 def self.file_usable?
  ...
  def bytes(path, &block)
    ...
    #length = AWS::S3::S3Object.size(path,'em-ftpd-trial/mugdha/Dir1')
...
=end
  ...
  def delete_dir(path, &block)
  	AWS::S3::Base.establish_connection!(:access_key_id => $key, :secret_access_key => $skey)  #here propery connection done  
  	AWS::S3::S3Object.delete path, 'em-ftpd-trial/mugdha'                                      #here directory gets deleted
    yield true
  end
